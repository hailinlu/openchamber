...
async function resolveTargetInstance({
  ...
  if (options.all && requireAll) {
    if (running.length === 0) {
      throw new Error('No running GridForge instance found. Start one with `gridforge serve`.');
    }
    return running;
  }
  ...
  if (options.explicitPort) {
      ...
      if (rejectDesktopRuntime) {
        ...
        if (!attachability.attachable) {
          if (attachability.reason === 'desktop') {
            throw new Error(
              `Port ${options.port} is used by GridForge Desktop app. Tunnel attach requires a CLI instance from \`gridforge serve\`.`
            );
          }
          throw new Error(
            `Port ${options.port} is not an attachable GridForge tunnel instance. Ensure it is healthy and running GridForge CLI runtime.`
          );
        }
      }
      return found;
    ...
    throw new Error(`No running GridForge instance found on port ${options.port}.`);
  }
  ...
  if (rejectDesktopRuntime) {
    ...
    if (attachableEntries.length > 1) {
      ...
      throw new Error(`Multiple attachable GridForge instances found: ${ports}. Use --port <port> or --all.`);
    }
    ...
    if (sawDesktop) {
      throw new Error('Only GridForge Desktop instance(s) detected. Tunnel attach requires a CLI instance from `gridforge serve`.');
    }

    throw new Error('No attachable GridForge instance found. Start one with `gridforge serve`.');
  }
  ...
  throw new Error(`Multiple GridForge instances found: ${ports}. Use --port <port> or --all.`);
}
...
async function handleTunnelProfileSubcommand(options, action, { boldText, ensureTunnelProfilesMigrated }) {
  ...
  if (!sub) {
    ...
    if (!isQuietMode(options)) {
      ...
      clackLog.step('List profiles: `gridforge tunnel profile list`');
      clackLog.step('Show one profile: `gridforge tunnel profile show --name <name>`');
      clackLog.step('Add profile: `gridforge tunnel profile add --provider cloudflare --mode managed-remote --name <name> --hostname <host> --token <token>`');
      clackLog.step('Remove profile: `gridforge tunnel profile remove --name <name>`');
  ...
  if (sub === 'add') {
    ...
    logStatus('info', '[START_PROFILE]', `gridforge tunnel start --profile ${added.name}`);
    clackOutro('');
    return;
  }
  ...
  throw new TunnelCliError(
    `Unknown tunnel profile subcommand '${sub}'.${hint} Use 'gridforge tunnel help'.`,
    ...
    switch (subcommand) {
      ...
      case 'doctor': {
        ...
        if (desktopUnavailablePorts.length > 0) {
          clackLog.message('Only CLI instances (gridforge serve) support tunneling.');
        }

        if (cliPorts.length === 0 && unavailablePorts.length === 0) {
          logStatus('warning', 'No running instances found', 'Start one with `gridforge serve`.');
          clackOutro('No ports available');
          return;
        }
        if (cliPorts.length === 0) {
          logStatus('warning', 'No CLI instances available for tunneling', 'Start one with `gridforge serve`.');
          ...
          if (isReady) {
            ...
            if (isManagedRemote && (hasPortOrOriginIssue || hasTokenIssue)) {
              troubleshootingHints.push({
                ...
                code: '[PORT_MISMATCH]',
                lines: [
                  'Cloudflare target must match the active GridForge CLI port.',
                  'Example: `http://127.0.0.1:<port>`',
                  'If CLI picked a different port, update Cloudflare or run `gridforge serve --port <port>`.',
                ],
              });
            }
      ...
      case 'start': {
        ...
        if (!options.explicitPort && canPrompt(options)) {
          ...
          if (runningInstances.length > 1) {
            ...
            if (safeInstances.length === 0) {
              throw new TunnelCliError(
                'All discovered GridForge instance ports are browser-unsafe. Start or target a safe port (3000, 5173, 8080, or high ephemeral).',
                EXIT_CODE.USAGE_ERROR,
              );
            }
            ...
            if (attachableSafeInstances.length === 0) {
              throw new TunnelCliError(
                'No attachable GridForge CLI instances found on safe ports. Start one with `gridforge serve --port 3000`.',
                EXIT_CODE.USAGE_ERROR,
              );
            }

            const selectedPort = await clackSelect({
              message: 'Select GridForge instance port',
        ...
        if (instance?.autoStarted) {
          ...
          healthProgress?.start(`Waiting for GridForge on port ${instance.port} to become healthy (up to 60s)...`);
          ...
          const healthy = await waitForServerHealth(instance.port, {
            ...
            onTick({ elapsedMs, complete }) {
              ...
              if (delta > 0) {
                ...
                healthProgress.message(`Waiting for GridForge health (${progressedSeconds}s / 60s)...`);
              }
          ...
          if (!healthy) {
            healthProgress?.stop('GridForge is still starting');
            throw new Error(
              `GridForge on port ${instance.port} is still starting after 60s. Startup time can vary by machine performance. ` +
              `Wait another minute, then check health with \`curl -fsS ${buildLocalUrl(instance.port, '/health')}\`. ` +
              `If health is OK, retry tunnel start with \`gridforge tunnel start --port ${instance.port}\`. ` +
              `For diagnostics run \`gridforge logs -p ${instance.port}\`.`
            );
          }
        ...
        try {
          ...
          if (error instanceof Error && /\/api\/openchamber\/tunnel\/start/.test(error.message) && /timed out/.test(error.message)) {
            ...
            throw new Error(
              `Tunnel start timed out after 60s. cloudflared may still be starting; check with \`gridforge tunnel status --port ${instance.port}\`. Run \`gridforge logs -p ${instance.port}\` for details.`
            );
          }
          spin?.error('Tunnel start failed');
          const message = error instanceof Error ? error.message : String(error);
          throw new Error(`${message} Run \`gridforge logs -p ${instance.port}\` for details.`);
        }
        ...
        if (!response.ok || !body?.ok) {
          ...
          const userError = isCloudflareTimeout
            ...
            : baseError;
          throw new Error(`${userError} Run \`gridforge logs -p ${instance.port}\` for details.`);
        }
        ...
        if (isJsonMode(options)) {
          ...
          const optionalTips = [
            { line: 'Check status', detail: 'gridforge tunnel status' },
            { line: 'Stop tunnel', detail: 'gridforge tunnel stop' },
          ...
          if (!selectedProfile && mode === 'managed-remote' && typeof hostname === 'string' && hostname.trim().length > 0) {
            ...
            optionalTips.push({ line: 'Start from saved profile', detail: 'gridforge tunnel start --profile <name>' });
          }
      ...
      default: {
        ...
        const hint = suggestion ? ` Did you mean '${suggestion}'?` : '';
        throw new TunnelCliError(
          `Unknown tunnel subcommand '${subcommand}'.${hint} Use 'gridforge tunnel help'.`,
          EXIT_CODE.USAGE_ERROR
        );
      }
    }
