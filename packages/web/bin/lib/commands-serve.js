...
import { discoverGridforgeInstanceOnPort, isDesktopRuntimeForPort } from './cli-lifecycle.js';
...
async function serveCommand(options) {
    ...
    if (targetPort !== 0) {
      const existingInstance = await discoverGridforgeInstanceOnPort(targetPort, { host: effectiveHost });
      if (existingInstance?.runtime === 'desktop') {
        throw new Error(
          `Port ${targetPort} is used by GridForge Desktop app. Choose another port or stop the desktop app.`
        );
      }
      if (existingInstance) {
        const pidSuffix = Number.isFinite(existingInstance.pid) ? ` (PID: ${existingInstance.pid})` : '';
        if (existingInstance.source === 'probe') {
          throw new Error(`OpenChamber is already running on port ${targetPort}. Use \`gridforge status\` or \`gridforge stop --port ${targetPort}\`.`);
        }
    ...
    if (!effectiveUiPassword && !options.suppressUiPasswordWarning) {
      ...
      const warningLine = 'GRIDFORGE_UI_PASSWORD is not set';
      const warningDetail = networkExposed
        ...
        : 'browser UI is unsecured. Use --ui-password or GRIDFORGE_UI_PASSWORD.';
      if (showOutput) {
    ...
    if (options.foreground) {
      ...
      if (effectiveUiPassword) {
        process.env.GRIDFORGE_UI_PASSWORD = effectiveUiPassword;
      }
      process.env.GRIDFORGE_HOST = effectiveHost;
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting GridForge on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    const child = spawn(runtimeBin, serverArgs, {
      ...
      env: {
        ...
        ...(effectiveUiPassword ? { GRIDFORGE_UI_PASSWORD: effectiveUiPassword } : {}),
        ...(options.apiOnly === true ? { GRIDFORGE_API_ONLY: 'true' } : {}),
        ...(process.env.OPENCODE_SKIP_START ? { OPENCHAMBER_SKIP_OPENCODE_START: process.env.OPENCODE_SKIP_START } : {}),
      },
    });

    child.unref();
    serveSpin?.start(`Starting GridForge on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    const serveResult = {
      ...
      logs: `gridforge logs -p ${resolvedPort}`,
      launchMode: 'daemon',
    };
