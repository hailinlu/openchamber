...
const NOTIFICATION_DEDUPE_STORAGE_PREFIX = 'gridforge-notification-claim:';
...
const notifyWithServiceWorker = async (payload?: NotificationPayload): Promise<boolean> => {
  ...
  try {
    await registration.showNotification(payload?.title ?? 'GridForge', {
      body: payload?.body,
      tag: payload?.tag,
    });
    return true;
...
const notifyWithWebAPI = async (payload?: NotificationPayload): Promise<boolean> => {
  ...
  try {
    ...
    new Notification(payload?.title ?? 'GridForge', {
      body: payload?.body,
      tag: payload?.tag,
    });
    return true;
