...
export const createConfiguredWebAPIs = () => {
  const apiBaseUrl = typeof window.__GRIDFORGE_API_BASE_URL__ === 'string'
    ? window.__GRIDFORGE_API_BASE_URL__.trim()
    : '';
  const clientToken = typeof window.__GRIDFORGE_CLIENT_TOKEN__ === 'string'
    ? window.__GRIDFORGE_CLIENT_TOKEN__.trim()
    : '';
  const localOrigin = typeof window.__GRIDFORGE_LOCAL_ORIGIN__ === 'string'
    ? window.__GRIDFORGE_LOCAL_ORIGIN__.trim()
    : '';
  ...
  setRuntimeExtraHeaders(window.__GRIDFORGE_RUNTIME_HEADERS__ || null);
  ...
  const relayHostId = (window as typeof window & { __GRIDFORGE_RELAY_HOST_ID__?: string }).__GRIDFORGE_RELAY_HOST_ID__;
