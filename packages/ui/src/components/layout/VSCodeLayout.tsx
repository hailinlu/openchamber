...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __GRIDFORGE_CONNECTION__?: { status?: string } }).__GRIDFORGE_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('gridforge.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('gridforge.updateSessionEditorTitle', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __GRIDFORGE_CONNECTION__?: { status?: string } }).__GRIDFORGE_CONNECTION__?.status
    ...
    window.addEventListener('gridforge:connection-status', handler as EventListener);
    return () => window.removeEventListener('gridforge:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('gridforge:navigate', handler as EventListener);
    return () => window.removeEventListener('gridforge:navigate', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    const runBootstrap = async () => {
      ...
      try {
        ...
        if (debugEnabled) console.log('[GridForge][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[GridForge][VSCode][bootstrap] post-load', {
