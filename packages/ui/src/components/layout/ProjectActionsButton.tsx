...
const AUTO_DISCOVER_ACTION_ID = '__gridforge_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<GridforgeProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<GridforgeProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: GridforgeProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: GridforgeProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<GridforgeProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: GridforgeProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: GridforgeProjectAction, toggleStopIfRunning = false) => {
