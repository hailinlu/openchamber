...
const useExternalLinkInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (anchor.getAttribute('data-gridforge-file-link') === 'true') {
        return;
      }
...
const FILE_LINK_SELECTOR = '[data-gridforge-file-link="true"]';
const BLOCK_PATH_TOKEN_ATTR = 'data-gridforge-block-path-token';
const BLOCK_PATH_TOKEN_SELECTOR = `[${BLOCK_PATH_TOKEN_ATTR}]`;
const CODE_BLOCK_PATH_SCANNED_ATTR = 'data-gridforge-block-paths-scanned';
...
// `data-gridforge-block-path-token`. `annotateFileLinks` then promotes those
...
// `data-gridforge-block-paths-scanned` once processed so the walk is not
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const clearFileLinkAttributes = (candidate: HTMLElement) => {
      candidate.removeAttribute('data-gridforge-file-link');
      candidate.removeAttribute('data-gridforge-file-ref');
      candidate.removeAttribute('data-gridforge-file-path');
    ...
    const annotateFileLinks = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        void existsPromise.then((exists) => {
          ...
          candidate.setAttribute('data-gridforge-file-link', 'true');
          candidate.setAttribute('data-gridforge-file-ref', latestRawCandidate);
          candidate.setAttribute('data-gridforge-file-path', latestResolved.resolvedPath);
    ...
    const openFileReference = async (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-gridforge-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-gridforge-file-link') !== 'true') {
        return;
      }
