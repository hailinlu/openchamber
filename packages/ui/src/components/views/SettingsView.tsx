...
import type { GridforgeSection } from '@/components/sections/gridforge/types';
import { GridforgePage } from '@/components/sections/gridforge/GridforgePage';
...
const SETTINGS_DETAIL_HISTORY_KEY = '__gridforgeSettingsDetail';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  const isWindows = React.useMemo(() => {
    return isDesktopShell() && typeof window !== 'undefined'
      && (window as unknown as { __GRIDFORGE_PLATFORM__?: string }).__GRIDFORGE_PLATFORM__ === 'win32';
  }, []);
  ...
  const gridforgeSectionBySlug: Partial<Record<SettingsPageSlug, GridforgeSection>> = React.useMemo(() => ({
    appearance: 'visual',
    chat: 'chat',
    shortcuts: 'shortcuts',
    sessions: 'sessions',
  ...
  const openSearchResult = React.useCallback((result: SettingsSearchResult) => {
    ...
    if (result.id === 'plugins.create' && typeof window !== 'undefined') {
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('gridforge:settings-open-plugin-add'));
      }, 50);
    }
  ...
  const renderPageContent = React.useCallback((slug: SettingsPageSlug) => {
    ...
    switch (slug) {
      ...
      case 'voice':
      case 'tunnel': {
        const section = gridforgeSectionBySlug[slug] ?? 'visual';
        return <GridforgePage section={section} />;
      }
      default:
        return <SettingsHome onOpen={openPage} />;
    }
  }, [gridforgeSectionBySlug, openPage, renderUnavailable, runtimeCtx]);
