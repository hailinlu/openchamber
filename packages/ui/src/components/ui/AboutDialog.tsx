    ...
    const fetchVersion = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (typeof data.gridforgeVersion === 'string' && data.gridforgeVersion.trim()) {
            setVersion(data.gridforgeVersion);
            return;
          }
        }
      } catch {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xs p-6">
        <div className="flex flex-col items-center text-center space-y-4">
          <GridforgeLogo width={64} height={64} />

          <div className="space-y-1">
            <h2 className="text-lg font-semibold">GridForge</h2>
            <div className="space-y-0.5 typography-meta text-muted-foreground">
              {displayVersion && (
                <p>{t('aboutDialog.gridforgeVersionLabel', { version: displayVersion })}</p>
              )}
              {openCodeVersion && (
          ...
          <div className="flex flex-col items-center gap-2 pt-2">
            <div className="flex items-center justify-center gap-4">
              <a
                href="https://github.com/hailinlu/gridforge"
                target="_blank"
            ...
            <a
              href="https://x.com/gridforge_dev"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 typography-meta text-muted-foreground hover:text-foreground transition-colors"
            >
