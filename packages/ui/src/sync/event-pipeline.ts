...
const normalizeGridforgeSessionStatus = (payload: Event): Event | null => {
  ...
  if (record.type !== "gridforge:session-status") return null
  ...
  return {
    id: typeof record.id === "string" && record.id.length > 0
      ? record.id
      : `gridforge-status-${sessionID}-${Date.now()}`,
    type: "session.status",
    properties: {
      sessionID,
      status,
...
const normalizeEventType = (payload: Event): Event => {
  const normalizedGridforgeStatus = normalizeGridforgeSessionStatus(payload)
  if (normalizedGridforgeStatus) {
    return normalizedGridforgeStatus
  }
...
export function createEventPipeline(input: EventPipelineInput): EventPipeline {
  ...
  if (typeof globalThis.window !== "undefined") {
    globalThis.window.addEventListener("gridforge:system-resume", onSystemResume)
  ...
  const cleanup = () => {
    ...
    if (typeof globalThis.window !== "undefined") {
      globalThis.window.removeEventListener("gridforge:system-resume", onSystemResume)
