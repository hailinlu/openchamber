...
import { getWorktreeSetupWaitEnabled } from "@/lib/gridforgeConfig"
...
export type SessionUIState = {
  ...
  clearError: () => void
  markSessionAsGridforgeCreated: (sessionId: string) => void
  isGridforgeCreatedSession: (sessionId: string) => boolean
  getContextUsage: (contextLimit: number, outputLimit: number) => SessionContextUsage | null
  initializeNewGridforgeSession: (sessionId: string, agents: unknown[]) => void
...
export async function materializeOpenDraftSession(selection: {
  ...
  store.initializeNewGridforgeSession(created.id, configState.agents ?? [])
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsGridforgeCreated: (sessionId) =>
    set((s) => {
  ...
  isGridforgeCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
