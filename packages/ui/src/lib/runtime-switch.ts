...
const RUNTIME_ENDPOINT_CHANGED_EVENT = 'gridforge:runtime-endpoint-changed';

let activeApiBaseUrl = '';
let activeRuntimeKey = '';

const setWindowRuntimeValue = <K extends '__GRIDFORGE_API_BASE_URL__' | '__GRIDFORGE_CLIENT_TOKEN__' | '__GRIDFORGE_RUNTIME_HEADERS__'>(
  runtimeWindow: typeof window & {
    __GRIDFORGE_API_BASE_URL__?: string;
    __GRIDFORGE_CLIENT_TOKEN__?: string;
    __GRIDFORGE_RUNTIME_HEADERS__?: Record<string, string>;
  },
  key: K,
...
const readInjectedApiBaseUrl = (): string => {
  ...
  const injected = (window as typeof window & { __GRIDFORGE_API_BASE_URL__?: string }).__GRIDFORGE_API_BASE_URL__;
...
export const switchRuntimeEndpoint = (options: { apiBaseUrl: string; clientToken?: string | null; runtimeKey?: string | null; requestHeaders?: Record<string, string> | null; relay?: RelayRuntimeDescriptor | null }): void => {
  ...
  if (typeof window !== 'undefined') {
    ...
    setWindowRuntimeValue(runtimeWindow, '__GRIDFORGE_API_BASE_URL__', apiBaseUrl);
    setWindowRuntimeValue(runtimeWindow, '__GRIDFORGE_CLIENT_TOKEN__', options.clientToken || undefined);
    setWindowRuntimeValue(runtimeWindow, '__GRIDFORGE_RUNTIME_HEADERS__', options.requestHeaders || undefined);
  }
