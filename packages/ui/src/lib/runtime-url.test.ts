...
describe('createRuntimeUrlResolver', () => {
  ...
  test('uses realtime base URL for SSE and WebSocket URLs', () => {
    ...
    expect(urls.sse('/api/gridforge/events')).toBe('https://realtime.example/api/gridforge/events');
  ...
  test('uses injected desktop API base URL for packaged WebSocket URLs', () => {
    withWindow({
      location: { origin: 'gridforge-ui://app', href: 'gridforge-ui://app/index.html' },
      __GRIDFORGE_API_BASE_URL__: 'http://127.0.0.1:57123',
    }, () => {
      const urls = createRuntimeUrlResolver({});
  ...
  test('routes realtime URLs through local desktop proxy when runtime headers are configured', () => {
    ...
    try {
      withWindow({
        location: { origin: 'gridforge-ui://app', href: 'gridforge-ui://app/index.html' },
        __GRIDFORGE_API_BASE_URL__: 'https://remote.example',
        __GRIDFORGE_LOCAL_ORIGIN__: 'http://127.0.0.1:57123',
      }, () => {
        const urls = createRuntimeUrlResolver({});
  ...
  test('reads injected desktop API base URL at call time', () => {
    withWindow({
      ...
      const urls = createRuntimeUrlResolver({});
      (window as typeof window & { __GRIDFORGE_API_BASE_URL__?: string }).__GRIDFORGE_API_BASE_URL__ = 'http://127.0.0.1:57123';
  ...
  test('adds short-lived URL auth query to realtime and authenticated asset URLs only', () => {
    ...
    try {
      ...
      expect(urls.sse('/api/gridforge/events')).toBe(
        'https://api.example/api/gridforge/events?oc_url_token=oc_url_secret',
      );
      expect(urls.websocket('/api/global/event/ws', { lastEventId: 'evt-1' })).toBe(
  ...
  test('does not put the long-lived client token in URLs', () => {
    ...
    try {
      const urls = createRuntimeUrlResolver({ apiBaseUrl: 'https://api.example' });
      expect(urls.sse('/api/gridforge/events')).toBe('https://api.example/api/gridforge/events');
