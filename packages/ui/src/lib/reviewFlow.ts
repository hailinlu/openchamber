...
const requestChatForceScrollBottom = (sessionId: string): void => {
  ...
  window.dispatchEvent(new CustomEvent('gridforge:chat-force-scroll-bottom', {
    detail: { sessionId },
  }));
};
...
const createOrReuseReviewSession = async (originalSessionID: string, directory: string, expectedRuntimeKey?: string): Promise<Session> => {
  ...
  if (existingReviewID) {
    ...
    await patchSessionMetadata(originalSessionID, directory, (metadata) => {
      const next = { ...metadata };
      const gridforge = next.gridforge;
      if (gridforge && typeof gridforge === 'object' && !Array.isArray(gridforge)) {
        const rest = { ...(gridforge as Record<string, unknown>) };
