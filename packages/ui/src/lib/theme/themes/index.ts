...
import gridforgeLightRaw from './fields-of-the-shire-light.json';
import gridforgeDarkRaw from './fields-of-the-shire-dark.json';

const flexokiLightTheme = withPrColors(flexokiLightRaw as Theme);
const flexokiDarkTheme = withPrColors(flexokiDarkRaw as Theme);
const gridforgeLightTheme = withPrColors(gridforgeLightRaw as Theme);
const gridforgeDarkTheme = withPrColors(gridforgeDarkRaw as Theme);
...
export const themes: Theme[] = [
  ...
  flexokiDarkTheme,
  ...presetThemes.filter(
    (theme) => theme.metadata.id !== 'gridforge-light' && theme.metadata.id !== 'gridforge-dark',
  ),
];
