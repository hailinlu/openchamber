...
declare global {
  interface Window {
    __GRIDFORGE_STARTUP_TRACE__?: StartupTraceEvent[];
    __GRIDFORGE_STARTUP_TRACE_START__?: number;
  }
}
...
const enabled = () => {
  ...
  try {
    ...
    return params.get('startupTrace') === '1' || window.localStorage?.getItem('GRIDFORGE_STARTUP_TRACE') === '1';
  } catch {
    return false;
  }
};
...
export const markStartupTrace = (name: string, data?: Record<string, unknown>) => {
  ...
  window.__GRIDFORGE_STARTUP_TRACE_START__ ??= now;
  window.__GRIDFORGE_STARTUP_TRACE__ ??= [];
  window.__GRIDFORGE_STARTUP_TRACE__.push({
    t: Math.round(now - window.__GRIDFORGE_STARTUP_TRACE_START__),
    name,
    ...(data ? { data } : {}),
  });
  if (window.__GRIDFORGE_STARTUP_TRACE__.length > MAX_STARTUP_TRACE_EVENTS) {
    window.__GRIDFORGE_STARTUP_TRACE__.splice(
      0,
      window.__GRIDFORGE_STARTUP_TRACE__.length - MAX_STARTUP_TRACE_EVENTS,
    );
  }
};

const getStartupTraceSummary = () => {
  const trace = typeof window !== 'undefined' ? window.__GRIDFORGE_STARTUP_TRACE__ ?? [] : [];
...
if (typeof window !== 'undefined') {
  (window as typeof window & { __GRIDFORGE_STARTUP_TRACE_SUMMARY__?: typeof getStartupTraceSummary })
    .__GRIDFORGE_STARTUP_TRACE_SUMMARY__ = getStartupTraceSummary;
}
