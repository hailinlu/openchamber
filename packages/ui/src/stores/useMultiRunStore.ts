...
import { getWorktreeSetupWaitEnabled, saveWorktreeSetupCommands } from '@/lib/gridforgeConfig';
...
const registerCreatedSession = (session: Session, directory: string): Session => {
  ...
  useSessionUIStore.getState().markSessionAsGridforgeCreated(session.id);
