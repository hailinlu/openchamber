...
const TRAY_ACTION_EVENT = 'gridforge:tray-action';
...
type TraySnapshot = {
  ...
  // Active instance label (e.g. "Local GridForge" or a remote host name) so
...
// the existing `gridforge:open-session` / `gridforge:open-draft-session`
...
// the local origin shows as "Local GridForge"; a remote host shows its
// configured name. Async because the host config is read over IPC.
const resolveInstanceName = async (): Promise<string> => {
  try {
    if (isDesktopLocalOriginActive()) return 'Local GridForge';
    const localOrigin = (window as unknown as { __GRIDFORGE_LOCAL_ORIGIN__?: string }).__GRIDFORGE_LOCAL_ORIGIN__
      || window.location.origin;
    const runtimeApiBaseUrl = getRuntimeApiBaseUrl();
    if (runtimeApiBaseUrl && locationMatchesHost(runtimeApiBaseUrl, localOrigin)) return 'Local GridForge';
...
export const useTraySync = (): void => {
  ...
  React.useEffect(() => {
    ...
    const bridge = (window as unknown as { __GRIDFORGE_DESKTOP__?: DesktopBridgeGlobal }).__GRIDFORGE_DESKTOP__;
