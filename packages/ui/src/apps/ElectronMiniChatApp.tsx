...
const MINI_CHAT_PRESENCE_CHANNEL = 'gridforge:mini-chat-presence';
...
const MiniChatBootstrap: React.FC<{ config: MiniChatConfig }> = ({ config }) => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('gridforge:open-session', onOpenSession);
    return () => window.removeEventListener('gridforge:open-session', onOpenSession);
