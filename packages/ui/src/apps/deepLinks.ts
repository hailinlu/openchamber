/**
 * GridForge deep-link vocabulary — the single source of truth for the `gridforge://`
...
export const DEEP_LINK_SCHEME = 'gridforge';
...
const segmentsOf = (url: URL): string[] => {
  // Custom-scheme URLs put the first route token in `host` (gridforge://session/<id>),
  // but be tolerant of authority-less forms (gridforge:/session/<id>) where it lands in
...
/**
 * Parse a raw `gridforge://…` string into a typed intent, or `null` if it isn't a
 * recognised GridForge deep link. Tolerant by design: unknown routes return `null`
...
/**
 * Build a canonical `gridforge://…` URL for an intent. Used by anything that needs to hand
