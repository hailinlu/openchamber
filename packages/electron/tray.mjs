...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'GridForge — no active sessions';
  const bits = [];
  ...
  return `GridForge — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    template.push(
      ...
      { label: 'Show GridForge', click: () => onAction({ type: 'show-main-window' }) },
      { type: 'separator' },
      { label: 'Quit GridForge', click: () => onAction({ type: 'quit' }) },
    );
