...
- **桌面集成最终态**:Rust 后端嵌入 Tauri 进程(同进程内嵌 web 服务器,UI 走 loopback,单二进制)
- **桌面壳优先 + sidecar 过渡**(2026-07-14 调整):桌面壳提前到后端迁移之前开发,过渡期 Tauri 壳以 **sidecar 方式 spawn 现有 `@openchamber/web` CLI**(`openchamber serve --port <p> --foreground`)。理由:
  - 桌面壳主体工作量(原生集成层:窗口/托盘/菜单/深链/自动更新/SSH)与后端语言无关,不依赖后端迁移进度
  - Tauri 原生能力上限是最大未知风险,早日验证 > 拖到最后
  - SSH 重写(`ssh-manager.mjs` 49KB)是独立大工程,提前可与后端迁移并行
  - 代价:后端迁移完成后需做一次"sidecar → 进程内嵌"二次集成(工作量小,仅改启动逻辑)
...
### 阶段 4A:Tauri 桌面壳优先(sidecar 过渡,4-6 周)

**可提前启动,不依赖阶段 1-3。** 2026-07-14 决策调整:桌面壳优先于后端迁移。
...
│  ┌───────────────────────────────────────────────────┐  │
│  │  原生集成层(Tauri/Rust)                            │  │
│  │  窗口/托盘/菜单/深链/自动更新/SSH/通知              │  │
│  └───────────────┬───────────────────────────────────┘  │
│                  │ spawn + 监听端口                       │
│  ┌───────────────▼───────────────────────────────────┐  │
│  │  sidecar: @openchamber/web CLI (现有 Node 后端)     │  │
│  │  openchamber serve --port <p> --foreground          │  │
│  └───────────────┬───────────────────────────────────┘  │
│                  │ loopback 127.0.0.1:<port>              │
│  ┌───────────────▼───────────────────────────────────┐  │
│  │  WebView(现有 React UI,零改动)                     │  │
...
**工作分解:**
- `oc-tauri` 初始化为真正 Tauri 应用(`tauri-cli`),WebView 加载 loopback
- sidecar 管理:启动时 spawn `openchamber serve --foreground`、端口探测/就绪门、优雅退出时 kill sidecar
...
### 阶段 4B:进程内嵌(后端迁移完成后,0.5-1 周)

**前置:阶段 1-3 完成(后端 Rust 化)。**

- Tauri 壳从 sidecar Node 切换为进程内嵌 `oc-server`(axum)
- 删除 sidecar spawn/端口探测逻辑,改为进程内调用
- 原生集成层零改动(与后端嵌入方式解耦)
- 此时达到最终态:单二进制,内嵌 Rust 后端
...
| Electron→Tauri 能力缺口(SSH、托盘细节) | 中 | 阶段 4A 早期做能力对照表;`russh` 兜底;sidecar 过渡降低耦合风险 |
| sidecar→进程内嵌二次集成 | 低 | 阶段 4B 仅改启动逻辑,原生集成层与嵌入方式解耦 |
...
|---|---|
| 阶段 0 脚手架 | 1-2 周 |
| 阶段 4A 桌面壳(sidecar 过渡,可与阶段 1-3 并行) | 4-6 周 |
| 阶段 1 第一切片 | 2-3 周 |
| 阶段 2 实时层(深水区) | 3-4 周 |
| 阶段 3 功能模块(可并行) | 8-12 周 |
| 阶段 4B 进程内嵌 | 0.5-1 周 |
| 阶段 5 收尾 | 1-2 周 |
| **合计** | **约 5-8 个月**(1-2 人全职;阶段 4A 与阶段 1-3 可并行压缩) |
