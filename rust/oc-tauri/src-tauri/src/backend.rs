...
//!
//! Phase 4B: Tauri setup 按 `GRIDFORGE_SIDECAR` env 选择后端路径。
...
pub enum BackendHandle {
    ...
    InProcess(OcServer),
    /// sidecar 子进程 (GRIDFORGE_SIDECAR=1 回退路径)。
...
/// 决策: 是否走 sidecar 回退路径。
///
/// 环境变量 `GRIDFORGE_SIDECAR` 为 `1`/`true`/`TRUE` 时返回 true,
...
/// 纯解析: 给定 `GRIDFORGE_SIDECAR` 的原始字符串, 返回对应的 sidecar 决策。
///
...
mod tests {
    ...
    /// 序列化所有 mutate `GRIDFORGE_SIDECAR` 进程 env 的测试。
    ...
    fn use_sidecar_integration_with_env() {
        ...
        let _env = EnvGuard::capture("GRIDFORGE_SIDECAR");

        // unset 路径: 默认 false。
        std::env::remove_var("GRIDFORGE_SIDECAR");
        assert!(!use_sidecar(), "unset env should default to false");

        // 三个 truthy 值都应返回 true。
        for v in ["1", "true", "TRUE"] {
            std::env::set_var("GRIDFORGE_SIDECAR", v);
    ...
    /// RED 验证: panic 时 `EnvGuard::Drop` 必须还原 GRIDFORGE_SIDECAR。
    /// 使用 `catch_unwind` 触发 unwind, 然后断言 env 状态与 guard 创建前一致。
    /// 该测试在 `EnvGuard` 类型实现前编译失败 (类型未定义), 实现后 GREEN。
    #[test]
    fn env_guard_restores_gridforge_sidecar_on_panic() {
        // 串行化所有 mutating env 的测试, 避免外部干扰。
        let _lock = ENV_LOCK.lock().expect("ENV_LOCK poisoned by prior test panic");

        // 子用例 A: 起始状态 = 已设置值, 中途 panic, 期望还原为该值。
        let pre_existing = "pre-existing-marker";
        std::env::set_var("GRIDFORGE_SIDECAR", pre_existing);
        assert_eq!(
            std::env::var_os("GRIDFORGE_SIDECAR").as_deref().map(|s| s.to_str()),
            Some(Some(pre_existing)),
            "precondition: env must start as pre-existing value"
        );

        let panic_result = std::panic::catch_unwind(|| {
            let _guard = EnvGuard::capture("GRIDFORGE_SIDECAR");
            // 在 guard 持有期间 mutate env, 然后主动 panic。
            std::env::set_var("GRIDFORGE_SIDECAR", "mutated-then-panic");
        ...
        std::env::remove_var("GRIDFORGE_SIDECAR");
        assert!(
            std::env::var_os("GRIDFORGE_SIDECAR").is_none(),
        ...
        std::env::set_var("GRIDFORGE_SIDECAR", &original);
        let pre_bytes = std::env::var_os("GRIDFORGE_SIDECAR").expect("precondition: env must hold value");
        assert_eq!(
        ...
        let panic_result = std::panic::catch_unwind(|| {
            let _guard = EnvGuard::capture("GRIDFORGE_SIDECAR");
            std::env::set_var("GRIDFORGE_SIDECAR", "mutated-and-panicked");
            panic!("intentional panic for OsString roundtrip");
        });
        assert!(panic_result.is_err(), "catch_unwind should observe the panic");

        let post_bytes = std::env::var_os("GRIDFORGE_SIDECAR").expect("postcondition: env must hold restored value");
        assert_eq!(
